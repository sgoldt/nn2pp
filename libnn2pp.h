 ...
 * ----------
 * w :
 *     input-to-hidden unit weights of the net
 * v :
 *     hidden unit-to-output weights of the net
 * xis : (bs, N)
 *     inputs
 * g :
 *     activation function
 ...
 * ----------
 * w :
 *     input-to-hidden unit weights of the network
 * xis : (bs, N)
 *     NETWORK inputs
 * g :
 *     activation function
...
/**
 * Numerically computes the mse of a network with the given weights as half the
 * squared difference between the networks's output and the given ys.
 *
 * w : (K, N)
 *     input-to-hidden unit weights of the network
 * v : (K)
 *     hidden unit-to-output weights of the network
 * xis : (bs, N)
 *     input matrix of size (bs, N) on which the mse is evaluated.
 * ys : (bs, 1)
 *     the "true" outputs for the given xis.
 * g :
 *     activation function of the network to use.
 */
...
/**
 * Classifies the given outputs of an network.
 *
 * Parameters:
 * ----------
 * ys : (bs, 1)
 ...
 * ----------
 * w : (K, N)
 *     input-to-hidden layer weights of the network
 * v : (K)
 ...
 * STOP: double
 *     stopping criterion: generalisation error at which to stop the simulation.
 *
 * Returns:
 *  --------
 * weigths : (w0.shape)
...
void learn(mat& B0, vec& A0, mat& w, vec& v,
           ...
           double lr, double lr2, double wd, double sigma,
           vec steps, int bs,
           ...
           bool store=false, std::string log_fname="", double STOP=1e-12) {
  const int N = w.n_cols;
